...
/**
 * 가상 대기열 오픈런 부하 테스트 — 5000명 동시 접속.
 *
 * 도메인 게이트(/customer/**) + 용량 기반 입장 아키텍처를 "실제 사용자 여정 그대로" 흉내낸다:
 *   1) /customer/products 진입 (리다이렉트 수동 처리)
 *        - 여유(CPU<80% & 대기 줄 없음) → 200 즉시 입장
...
const BASE = 'http://hf4-alb-hinsight-1491375634.ap-northeast-2.elb.amazonaws.com';
const ENTRY = `${BASE}/customer/products`;        // 도메인 게이트 진입 경로
...
export const options = {
  // 응답 바디는 기본 폐기(5000 VU 메모리 절약). status 폴링만 바디를 유지(responseType:'text')
  discardResponseBodies: true,
  scenarios: {
    openrun_spike: {
      executor: 'per-vu-iterations',
      vus: 5000,            // 5000명 동시
      iterations: 1,        // 각자 1회 오픈런 여정
      maxDuration: '300s',  // 대기열 소진까지 여유
    },
  },
  thresholds: {
    http_req_failed: ['rate<0.05'],                    // 302/폴링은 실패 아님. 5xx/타임아웃만
    http_req_duration: ['p(95)<3000', 'p(99)<10000'],  // 개별 요청 지연(대기 sleep 은 제외됨)
  },
};
...
export default function () {
  ...
  const redirectPath = m ? decodeURIComponent(m[1]) : '/customer/products';
...
function browse() {
  for (const id of [30, 1, 5]) {
    const r = http.get(`${BASE}/customer/products/${id}`, { tags: { step: 'browse' } });
